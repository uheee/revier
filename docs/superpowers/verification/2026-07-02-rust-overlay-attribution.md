...
- Node：`v24.13.0`。沙盒外通过 `fnm` 初始化后可用。
- pnpm：`10.28.1`。通过 `fnm` 选择 Node 版本后执行 `pnpm.CMD`。
...
`& '<fnm>' exec --using-file pnpm.CMD test -- tests/unit/reviewIpc.test.ts`
...
`target/debug/revier-analysis.exe index build --repo <repo> --branch develop --db target/revier-runtime-smoke.duckdb --format json`

- 退出状态：0
- JSON 摘要：`status=completed`，`indexedCommitCount=74`，`indexedFileCount=393`。

`target/debug/revier-analysis.exe file-overlay --repo <repo> --db target/revier-runtime-smoke.duckdb --base <Base> --head <Head> --branch develop --file docs/superpowers/verification/2026-07-02-rust-overlay-attribution.md --format json`

- 退出状态：0
- JSON 摘要：`version=1`，`overlay.mode=range`，`overlay.rows=79`，`overlay.blocks=9`，`firstBlock=block-1`。

`target/debug/revier-analysis.exe trace-block --repo <repo> --db target/revier-runtime-smoke.duckdb --base <Base> --head <Head> --branch develop --file docs/superpowers/verification/2026-07-02-rust-overlay-attribution.md --block-id block-1 --format json`
...
- DB：`<repo>/target/revier-overlay-smoke-61977d03df2b4f1fb731a7f21eb6c620.duckdb`

`$env:DUCKDB_DOWNLOAD_LIB='1'; cargo run -q -p revier-analysis -- index build --repo <repo> --branch develop --db <DB> --format json`

- 退出状态：0
- JSON 摘要：`status=completed`，`indexedCommitCount=71`，`indexedFileCount=387`。

`$env:DUCKDB_DOWNLOAD_LIB='1'; cargo run -q -p revier-analysis -- file-overlay --repo <repo> --db <DB> --base <Base> --head <Head> --branch develop --file src/main/ipc/reviewIpc.ts --format json`
...
`$env:DUCKDB_DOWNLOAD_LIB='1'; cargo run -q -p revier-analysis -- trace-block --repo <repo> --db <DB> --base <Base> --head <Head> --branch develop --file src/main/ipc/reviewIpc.ts --block-id block-1 --format json`
...
Node 侧验证已补充完成：通过 `fnm` 进入 Node 24 / pnpm 10.28.1 环境后，`pnpm typecheck`、`pnpm test -- tests/unit/reviewIpc.test.ts` 和 `pnpm test` 均通过。
