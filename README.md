...
6. 用户点击相关提交时，Rust 生成提交级 Overlay Diff。
...
仓库索引数据库按“项目 + 分支”保存一份最近成功的分析快照，并以规范化结构保存文件块、作者归因和相关提交引用，不复制源码正文。再次打开项目或文件时优先读取缓存；分支 HEAD 变化会将快照标记为过期，不会自动混入新提交。此时需先点击“重新分析项目”，项目缓存有效时也可单独点击“重新分析当前文件”。所有载入操作都会在底部状态栏持续显示当前对象、阶段和耗时。
